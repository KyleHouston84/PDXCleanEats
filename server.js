...
function getRestaurants(response, query) {
    ...
    var optionIndex = query.indexOf('&');
    var lngLat = query.substring(0,optionIndex);
    var dis;
    if (optionIndex != -1) {
        dis = query.substring((optionIndex + 5),strLength);
    } else {
        dis = 1;
    }
    ...
    http.get("http://api.civicapps.org/restaurant-inspections/near/" + query + "?distance=" + dis + "&count=20000", function (res) {
